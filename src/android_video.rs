            ...
            .spawn(move || {
                let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| unsafe {
                    ...
                    if extractor.is_null() {
                        crate::android::log_android(5, "[RustTracker Video] Failed to create AMediaExtractor");
                        return;
                    }

                    use std::os::fd::{FromRawFd, IntoRawFd};
                    let Ok(file) = std::fs::File::open(&path_string) else {
                        crate::android::log_android(5, &format!("[RustTracker Video] Failed to open local file {}", path_string));
                    ...
                    if status != 0 {
                        crate::android::log_android(5, &format!("[RustTracker Video] Failed to set data source FD on AMediaExtractor (status {}) for {}", status, path_string));
                    ...
                    if codec.is_null() || video_track_idx.is_none() {
                        crate::android::log_android(4, &format!("[RustTracker Video] No suitable hardware video decoder found for {}", path_string));
                        AMediaExtractor_delete(extractor);
                        let _ = std::fs::File::from_raw_fd(raw_fd);
                        return;
                    }
                    ...
                    while !stop_token_for_video.load(Ordering::Relaxed) {
                        ...
                        if !eos_input {
                            ...
                            if in_idx >= 0 {
                                ...
                                if !in_buf.is_null() && buf_cap > 0 {
                                    ...
                                    if sample_size > 0 {
                                        let sample_pts = AMediaExtractor_getSampleTime(extractor).max(0) as u64;
                                        let sample_flags = AMediaExtractor_getSampleFlags(extractor);
                                        AMediaCodec_queueInputBuffer(codec, in_idx as usize, 0, sample_size as usize, sample_pts, sample_flags);
                        ...
                        if out_idx >= 0 {
                            ...
                            if info.size > 0 {
                                ...
                                let frame_len = info.size as usize;
                                if !out_buf.is_null() && out_size >= offset + frame_len {
                                    let src_slice = std::slice::from_raw_parts(out_buf.add(offset), frame_len);

                                    let w = (vid_w as usize).max(1);
                                    let h = (vid_h as usize).max(1);
                                    ...
                                    // Copy Y plane row by row with safe bounds
                                    for row in 0..h {
                    ...
                    AMediaExtractor_delete(extractor);
                    let _ = std::fs::File::from_raw_fd(raw_fd);
                    crate::android::log_android(3, "[RustTracker Video] Decoder cleanly shut down");
                }));
            })
            .is_ok()
    }
